...
showStuPersDetail: function* (next){
  ...
  UpDetails:function*(next){
    ...
    var uid=this.currentUser.useid;
    console.log(uid,"het");
    var queryString='update tb_stud set email="%s",fname="%s",mname="%s",summary="%s",fnumber="%s",foccu="%s",moccu="%s",dob="%s" where uid=%s';
     var query=util.format(queryString,email,fname,mname,summary,fnumber,foccu,moccu,dob,uid);
